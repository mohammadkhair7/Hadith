...
| Narrator explorer | bounded graph canvas + incremental expansion + hover cards + side panel (full bio, hadith list, assessments table); PyVis-style HTML export; research directory beneath the graph (`/narrators/directory`): list/filter/sort/search on name/alias, generation, rijal grade, death-year range, place, teacher/student, book, hadith topic, min mentions — click a row to load it in the graph |
