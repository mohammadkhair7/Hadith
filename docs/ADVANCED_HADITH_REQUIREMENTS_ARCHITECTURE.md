...
| Unified search | tabs: البحث في النصوص (keyword/exact/semantic/hybrid + facet filters) and اسأل الموسوعة (LLM-routed NL2SQL / NL2CYPHER / retrieval answer with citations — merged from the former standalone Ask page; `/ask` redirects to `/search?tab=ask`); generated SQL/Cypher shown collapsibly |
| Reader | TOC tree (root nodes auto-expanded when ≤3 roots; shamela TOCs generated by `ops/build_shamela_toc.py` from in-text heading lines كتاب/باب/فصل/[bracketed]/§-marked) + passage view (tashkeel display; shamela pages use markdown-style block rendering — styled headings + paragraphs via `pageBlocks()`, RAW-offset based so tashkeel toggle and matn highlighting are unaffected; shamela `\r` line breaks converted to `\n` in-place by `ops/fix_shamela_newlines.py`, offset-preserving), edition switcher for multi-edition works, prev/next, subjects chips, "find in Shamela/sunna" cross-link, translation display when available (§11) |
| Hadith detail | matn, isnad chain visual (mini-graph), takhrij panel (same hadith across collections), grades of chain narrators |
| Narrator explorer | bounded graph canvas + incremental expansion + hover cards + side panel (full bio, hadith list, assessments table); PyVis-style HTML export; research directory beneath the graph (`/narrators/directory`): list/filter/sort/search on name/alias, generation, rijal grade, death-year range, place, teacher/student, book, hadith topic, min mentions — 1000 rows/page (scrollable, prev/next + page-jump field; unfiltered pages cached and warmed at startup) — click a row to load it in the graph. Analytics أكثر الرواة ذكرًا (1000/page) and أشهر طرق الرواية (100/page) page through the FULL lists from the DB |
| **Timeline (الخط الزمني)** | suggested hadith-origination timeline on the hijri axis (negative = pre-hijra): dated-event lexicon (38 events, context-guarded regexes), seasonal anchors (رمضان/حج/عيد), companion narration windows (40 curated Companions, marfūʿ window capped at 11 AH); `hadith_dates` + `timeline_events` tables via `ops/analyze_timeline.py`; year chart, era-grouped events, companion windows, drill-down lists; full methodology & results in `docs/HADITH_TIMELINE_ANALYSIS.md` |
...
| **Admin › Narrator Management** (first tab) | merge duplicate narrator nodes (isnad mentions/aliases/assessments repointed, source names kept as aliases, scalar fields backfilled), create narrator, delete narrator with warning (links become unresolved, sanad text kept), add/remove relationship as manual overrides in `narrator_edges_manual` (layered onto derived isnad edges in `_neighbors`; isnad data never altered); every action audited in `admin_audit`; e.g. 74 duplicate عائشة أم المؤمنين nodes merged into one via `ops/merge_aisha_duplicates.py` |
...
GET  /narrators/directory?q_name=&generation=&grade=&place=&death_from=&death_to=
     &teacher=&student=&edition_id=&topic=&min_mentions=&sort=&limit=&offset=
GET  /narrators/directory/facets                → generations/grades/places/books
GET  /narrators/{id}/graph?depth=&cap=          → bounded subgraph (manual overrides applied)
...
GET  /admin/status
POST /admin/narrators/merge {target_id, source_ids} | POST /admin/narrators (create)
DELETE /admin/narrators/{id}                     → links unresolved, warning in UI
GET|POST /admin/narrators/relations | DELETE /admin/narrators/relations/{edge_id}
GET  /admin/narrators/audit
