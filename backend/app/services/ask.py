...
والنتائج المرفقة فقط، بالعربية الفصحى الواضحة.

أخرج الجواب بصيغة Markdown حصراً:
- استعمل عناوين قصيرة (##) عند تعدد المحاور، وقوائم نقطية للتعداد؛
- ضع نصوص الأحاديث في اقتباس (سطر يبدأ بـ >) وبيّن **الألفاظ المهمة** بالغامق؛
- استشهد بأرقام المصادر [1] [2] بين النصوص في موضع الاستشهاد؛
- اختم الجواب بقسم «## المراجع» يسرد كل مصدر مستشهد به في سطر مستقل:
  رقم المصدر ثم اسم الكتاب ورقم الحديث إن وُجد.

إن لم تكف النتائج فقل ذلك صراحة. لا تختلق أحاديث أو أرقاماً."""
...
def _compose_from_passages(question: str, items: list[dict]) -> str:
    ...
    return generate_text(
        f"السؤال: {question}\n\nالمقاطع:\n{ctx}\n\n"
        "أجب الآن بصيغة Markdown وجوباً:\n"
        "- ضع كل نصٍّ حديثي تقتبسه في سطر اقتباس يبدأ بـ > ؛\n"
        "- أبرز الألفاظ والأحكام المهمة بالغامق **هكذا**؛\n"
        "- استشهد بأرقام المصادر [1] [2] في مواضعها؛\n"
        "- اختم إلزامياً بقسم عنوانه «## المراجع» يسرد كل مصدر استشهدت به "
        "(رقمه ثم اسم الكتاب ورقم الحديث).\n\nالجواب (Markdown):",
...
def _compose_from_rows(question: str, engine_out: dict) -> str:
    ...
    return generate_text(
        ...
        "صِغ الجواب بالعربية اعتماداً على هذه النتيجة فقط، بصيغة Markdown: "
        "جملة أو جملتان، وجدول Markdown مختصر إذا كانت النتيجة قائمة قيم.",
        system=COMPOSER_SYSTEM)
