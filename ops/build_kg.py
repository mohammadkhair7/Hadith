    ...
    with pool.connection() as conn:
        ...
        for r in rows:
            ...
            chain_id = cur.execute("""
                ...
                VALUES (%s, 0, %s, %s) RETURNING chain_id
            """, (r["passage_id"], p.confidence, EXTRACTOR)).fetchone()["chain_id"]
            for hop in p.hops:
...
def resolve_entities() -> dict:
    """Stage 3-4 baseline: group mentions by normalized form; frequent forms
    become narrator entities with aliases; links get narrator_id."""
    with pool.connection() as conn:
        counts = conn.execute("""
            SELECT mention_norm, count(*) AS n,
                   (array_agg(mention_ar ORDER BY length(mention_ar)))[1] AS shortest_raw
            FROM isnad_links GROUP BY mention_norm HAVING count(*) >= %s
        """, (MIN_MENTION_COUNT,)).fetchall()

        created = 0
        for row in counts:
            existing = conn.execute(
                "SELECT narrator_id FROM narrator_aliases WHERE alias_norm=%s LIMIT 1",
                (row["mention_norm"],)).fetchone()
            if existing:
                nid = existing["narrator_id"]
            else:
                nid = conn.execute("""
                    INSERT INTO narrators (canonical_ar, canonical_norm)
                    VALUES (%s,%s) RETURNING narrator_id
                """, (row["shortest_raw"], row["mention_norm"])).fetchone()["narrator_id"]
                conn.execute("""
                ...
                """, (nid, row["shortest_raw"], row["mention_norm"]))
                created += 1
            conn.execute(
                "UPDATE isnad_links SET narrator_id=%s WHERE mention_norm=%s "
                "AND narrator_id IS NULL",
                (nid, row["mention_norm"]))
        conn.commit()
...
def build_graph(rebuild: bool = False) -> dict:
    ...
    with pool.connection() as conn:
        ...
        for row in narrators:
            ...
            conn.execute(
                ...
                f"MERGE (:Narrator {{narrator_id: {row['narrator_id']}, "
                f"name: '{name}'}}) $$) AS (v agtype)")
        conn.commit()
        ...
        for row in edges:
            conn.execute(
                ...
                f"MERGE (s)-[r:NARRATED_FROM]->(t) "
                f"SET r.count = {row['n']} $$) AS (v agtype)")
        conn.commit()
...
def main() -> None:
    ...
    if not args.skip_extract:
        ...
        print("extract:", json.dumps(stats["extract"]))
        stats["resolve"] = resolve_entities()
        print("resolve:", json.dumps(stats["resolve"]))
    stats["graph"] = build_graph(rebuild=args.rebuild_graph)
    print("graph:", json.dumps(stats["graph"]))
    pool.close()
